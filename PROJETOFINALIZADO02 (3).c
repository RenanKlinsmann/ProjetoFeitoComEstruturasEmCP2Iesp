// AUTORIA: LUCAS ALVES DA CRUZ LIRA E RENAN KLINSMANN ARAUJO MENDONCA
...
// Declarar o vetor de 15 elementos
struct a_cliente clientes[ALANA_CLIENTES];
int i;
int num_clientes = 0;

                    //Cria um vetor auxiliar
void cadastraCliente(struct a_cliente cliente) {
...
struct a_cliente leDados() {
    struct a_cliente auxiliar;
    printf("\n\n");
    printf("AUTORIA: ");
    fflush(stdin);
...
void removedor(int posicao) {
    int indice = posicao - 1;
    int i = 0;

    if ((posicao < 1) || (posicao > num_clientes)) {
        printf("\nPosicao invalida!\n");
        return;
    }


    if (posicao == num_clientes) {
        num_clientes--;
        return;
    }

    //Passa os elementos da lista antiga para a nova ate o elemento desejado
    for (i=indice; i<num_clientes; i++) {
...
int menuGerencia()
{
    ...
    struct a_cliente novo_cliente;
    int rem,opcaolistar,op6;
    ...
    do {
        ...
        switch(opcao_selecionada) {
            case 1: 
				...
				printf("======================================================================================\n");
				if (num_clientes == ALANA_CLIENTES) {
